...
void HAL_RTCEx_WakeUpTimerEventCallback(RTC_HandleTypeDef *hrtc)
{
	...
	if (HAL_RTC_GetTime(hrtc, &sTime,  RTC_FORMAT_BIN) == HAL_OK)
	{
		...
		uint8_t		timeStr[20];	//时间字符串 hh:mm:ss
		sprintf(timeStr,"%2d:%2d:%2d\n",sTime.Hours,sTime.Minutes,sTime.Seconds);  //自动加‘\0’

//		LCD_ShowStr(30,110, timeStr);  //在LCD上显示当前时间
		 OLED_ShowNum(0,0,sTime.Hours,2,12);
		 OLED_ShowNum(30,0,sTime.Minutes,2,12);
		 OLED_ShowNum(60,0,sTime.Seconds,2,12);
		if (isUploadTime)  //isUploadTime在文件usart.c中定义
